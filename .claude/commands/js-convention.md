...
Feed.init({
  getStockCards: () => getStockCards(),
  priceFormatter: (n) => formatAmount(n),
  rememberActivity: (data) => rememberFeedActivities(scope, [data]) > 0,
});
```
...
- `innerHTML`/`insertAdjacentHTML`은 **정적 마크업 + escaped 값**만 허용:
  ...
  el.insertAdjacentHTML('beforeend',
    '<span class="text-xs">' + escapeHtml(data.summary) + '</span>');
...
- ES 모듈 스코프 함수는 `onclick="fn()"` 에서 접근 불가.
- **해결**: `window.__feedToggleDetail = toggleDetail;` 처럼 글로벌 브릿지 등록.
...
| 비공개 함수 | `_` prefix + `camelCase` | `_appendFlexRow()`, `_removeToast()` |
| state 내부 callback | `_` prefix | `state._loadBalance` |
| DOM id | `kebab-case` | `chat-container`, `badge-sse` |
| CSS class | Tailwind 유틸리티 또는 `kebab-case` | `stock-card`, `cycle-divider` |
