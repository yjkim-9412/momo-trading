...
**Step 3. 리스크:보상 + 무효화 가격** — 목표가/손절가를 현재가·일봉·분봉에서 도출하고, 아이디어가 틀리는 가격을 손절가로 정의
  ...
  - 남은 timebox 안에 목표가 도달 가능성이 약하면 BUY가 아니라 HOLD
**Step 4. 종합 판단** — 과거 피드백, 현재 포지션, 과도한 FOMO 여부, 늦은 진입 여부까지 반영해 최종 결론
...
- 남은 timebox 안에 목표가·손절가·추세 완성 근거가 약하면 BUY가 아니라 HOLD입니다
...
- 실행 계약: 이 경로는 신규 매수 기회 탐색 전용이므로 최종 recommendation은 BUY 또는 HOLD만 사용하고 SELL은 사용하지 마세요
- HOLD를 선택한다면 거래대금 부족, 추격 매수, 추세 불일치, RR 부족, 지지선 회복 미확인 중 핵심 실패 사유를 분명히 반영하세요
...
### 코인 운영 제약
- 현재 타임박스: {timebox_hours}시간
- 포지션은 timebox 만료 시 자동 청산/정산
- 남은 보유 가능 시간 안에 목표가 도달 근거가 약하면 HOLD
...
- confidence: 이 매매가 손절 전에 목표가에 도달할 확률 (0.00~1.00)
- position_intent: NEW / ADD_ON_PYRAMID / ADD_ON_AVERAGE_DOWN / HOLD
- 강세 국면(BULL_RUN/ALTSEASON/THEME)에서는 추세·거래대금·RR이 충족되면 HOLD보다 BUY를 우선 검토하세요
- 거래대금이 약하거나 추격 매수 성격이 강하면 BUY보다 HOLD를 우선하세요
- timebox 안에 끝낼 수 없는 느린 추세면 BUY보다 HOLD를 우선하세요
- HOLD라면 거래대금 부족, 추격 매수, 추세 불일치, RR 부족, 지지선 회복 미확인 중 핵심 사유를 reason에 분명히 적으세요
