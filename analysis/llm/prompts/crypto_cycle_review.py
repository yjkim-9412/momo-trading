"""코인 자동 정산 회고 리포트 프롬프트"""

CRYPTO_CYCLE_REVIEW_SYSTEM = """당신은 빗썸 KRW 현물 자동매매 시스템의 자동 정산 회고 분석가입니다.
24시간 시장에서 직전 **정산 완료 구간**을 복기하고, 바로 이어질 다음 정산 윈도우에 반영할 피드백을 정리합니다.

## 분석 원칙
1. 직전 구간의 시장 흐름, 거래대금, 주도 코인/테마를 먼저 요약합니다.
2. 직전 구간의 자동 스캔/분석/체결/정산 성과를 구체적으로 평가합니다.
3. 다음 정산 윈도우에 바로 반영할 학습 포인트만 남깁니다.
4. 데이터에 없는 정보는 추측하지 않습니다.
5. 반드시 한국어로 답변합니다.

## 액션 아이템 규칙
- action_items는 다음 정산 윈도우부터 코드 레벨에서 바로 반영됩니다.
- 가장 중요한 3~5개만 제안하세요.
- param_name:
  - min_confidence
  - stop_loss_pct
...
CRYPTO_CYCLE_REVIEW_PROMPT = """## 코인 자동 정산 회고

### 회고 구간
- 시작: {period_started_at}
- 종료: {period_ended_at}
- 생성 유형: {report_source_label}
- 다음 적용 윈도우: {applied_cycle_label}
...
## 요청 사항
위 데이터를 바탕으로 직전 자동 정산 구간을 복기하고, 바로 이어질 다음 정산 윈도우에 반영할 회고 리포트를 작성하세요.
...
{{
  ...
  "performance_review": "자동 스캔/분석/체결/정산 성과 평가 (2~4문장)",
  "lessons_learned": "다음 정산 윈도우에 바로 반영할 핵심 학습 포인트",
  "next_cycle_plan": "다음 정산 윈도우의 우선 전략/주의 포인트",
  "top_picks": ["다음 정산 윈도우 관심 코인 1", "관심 코인 2"],
  "trade_evaluation": {{
    "total_trades": 0,
    "profitable_trades": 0,
    "loss_trades": 0,
