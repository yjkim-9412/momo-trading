/**
 * admin-feed.js — Activity feed rendering: stock-grouped cards, bubbles,
 * cycle dividers, LLM conversation detail, and scroll management.
 *
 ...
 *     setActivityCount:     (n) => {},
 *     incActivityCount:     () => {},
 *     getCurrentLogFilter:  () => 'ALL'|'SIGNAL',
 *     getAutoScroll:        () => bool,
 *     setAutoScroll:        (bool) => {},
 *     getMissedCount:       () => n,
 *     setMissedCount:       (n) => {},
 *     getLoadedActivityIds: () => Set,
 *     getChatContainer:     () => element,
 *     priceFormatter:       (n) => string,
 *     extractProductContext:(data) => object|null,
 *     renderProductBadge:   (ctx) => string,
 *     renderProductStrip:   (ctx) => string,
 *     onActivityAppended:   (data) => {},
 *   })
 */
(function () {
  'use strict';
  ...
  // appendActivity
  // ────────────────────────────────────────────
  Feed.appendActivity = function (data, options) {
    options = options || {};
    ...
    if (!skipStore) {
      if (typeof _config.rememberActivity === 'function') {
        ...
        var identity = Feed.getActivityIdentity(data);
    ...
    if (!symbol || isCycleActivity || isDailyPlan) {
      if (isCycleActivity && data.phase === 'START') {
        container.appendChild(Feed.createCycleDivider(data, true));
        ...
        container.appendChild(Feed.createCycleDivider(data, false));
      } else if (isLLMCall && !symbol) {
        container.appendChild(Feed.createBubble(data));
      } else {
        container.appendChild(Feed.createBubble(data));
      }
    } else {
      ...
      if (!card) {
        ...
        card = Feed.createStockCard(symbol, data);
        cards[cardKey] = card;
        container.appendChild(card.element);
      }

      Feed.addStepToCard(card, data);
      Feed.updateCardHeader(card);
    }
  ...
  // createCycleDivider
  // ────────────────────────────────────────────
  Feed.createCycleDivider = function (data, isStart) {
    var div = document.createElement('div');
    div.className = isStart ? 'cycle-divider cycle-start' : 'cycle-divider cycle-end';
    var summary = Admin.formatAdminAgentText(data.summary);
    if (isStart) {
  ...
  // createStockCard
  // ────────────────────────────────────────────
  Feed.createStockCard = function (symbol, firstActivity) {
    ...
    // Pick card icon from theme or default
    var cardIcon = (Admin.CardTheme && Admin.CardTheme.icon) || 'bar-chart-2';
    var badgeClass = (Admin.CardTheme && Admin.CardTheme.progressBadge) || 'bg-purple-900/40 text-purple-300';
    ...
    header.onclick = function () { Feed.toggleCardBody(card); };
  ...
  // addStepToCard
  // ────────────────────────────────────────────
  Feed.addStepToCard = function (card, data) {
    ...
    var progressKey = Admin.getProgressKey(data);
    ...
    if (data.phase === 'START') {
      ...
      var time = Admin.formatTime(data.created_at);
      var label = Admin.formatAdminAgentText((data.summary || '').replace(/\uC2DC\uC791$/, '').trim());
    ...
    var timeStr = Admin.formatTime(data.created_at);
    var typeColor = Admin.getTypeColor(data.activity_type);
    ...
    var html = '<span class="text-xs text-gray-500 shrink-0 w-14">' + timeStr + '</span>'
      ...
      + '<div class="text-xs">' + Admin.escapeHtml(Admin.formatAdminAgentText(data.summary)) + '</div>';

    // Meta line
    var meta = [];
    if (data.llm_provider) meta.push('<span class="text-' + typeColor + '-400">' + Admin.escapeHtml(data.llm_provider) + '</span>');
    ...
    if (data.detail) {
      ...
      var productStripHtml = (_config.renderProductStrip && _config.extractProductContext)
        ...
        : '';
      html += '<button onclick="event.stopPropagation(); Admin.Feed.toggleDetail(\'' + detailId + '\')" class="text-xs text-gray-500 hover:text-gray-300 mt-0.5 flex items-center gap-1">'
        ...
        + productStripHtml
        + (isLLMCall ? Feed.formatLLMConversation(data.detail) : '<div class="whitespace-pre-wrap break-all max-h-96 overflow-y-auto">' + Feed.formatDetail(data.detail, data.activity_type) + '</div>')
        + '</div>';
    }

    // Error
    if (data.error_message) {
      html += '<div class="text-xs text-red-400 mt-0.5">' + Admin.escapeHtml(Admin.formatAdminAgentText(data.error_message)) + '</div>';
    }
  ...
  // updateCardHeader
  // ────────────────────────────────────────────
  Feed.updateCardHeader = function (card) {
    ...
    var outcomeBg = (Admin.CardTheme && Admin.CardTheme.progressBadge) || 'bg-purple-900/40 text-purple-300';
    var totalMs = 0;

    // Theme colors
    var buyBg = (Admin.CardTheme && Admin.CardTheme.buyBadge) || 'bg-red-900/40 text-red-300';
    var sellBg = (Admin.CardTheme && Admin.CardTheme.sellBadge) || 'bg-blue-900/40 text-blue-300';
    var holdBg = (Admin.CardTheme && Admin.CardTheme.holdBadge) || 'bg-gray-700/60 text-gray-400';
    var errorBg = (Admin.CardTheme && Admin.CardTheme.errorBadge) || 'bg-yellow-900/40 text-yellow-300';
    ...
    Feed.applyFilterToCard(card);
  };

  // ────────────────────────────────────────────
  // applyFilterToCard
  // ────────────────────────────────────────────
  Feed.applyFilterToCard = function (card) {
  ...
  // toggleCardBody
  // ────────────────────────────────────────────
  Feed.toggleCardBody = function (card) {
  ...
  // createBubble
  // ────────────────────────────────────────────
  Feed.createBubble = function (data) {
    var div = document.createElement('div');
    ...
    if (data.detail) {
      ...
      var isLLMCall = data.activity_type === 'LLM_CALL';
      html += '<button onclick="Admin.Feed.toggleDetail(\'' + detailId + '\')" class="text-xs text-gray-500 hover:text-gray-300 mt-1 flex items-center gap-1">'
    ...
    if (data.error_message) {
      html += '<div class="text-xs text-red-400 mt-1">' + Admin.escapeHtml(Admin.formatAdminAgentText(data.error_message)) + '</div>';
    }
  ...
  // formatDetail
  // ────────────────────────────────────────────
  Feed.formatDetail = function (detail, activityType) {
    if (!detail) return '';
    try {
      var obj = Admin.transformAdminAgentValue(typeof detail === 'string' ? JSON.parse(detail) : detail);
      ...
      if (activityType === 'TIER1_ANALYSIS' && indicators) {
        ...
        return '<table class="tech-table mb-2">'
          ...
          + '</tr>'
          + '</table>'
          + '<pre class="whitespace-pre-wrap text-gray-400">' + Admin.escapeHtml(JSON.stringify(obj, null, 2)) + '</pre>';
      }
      return '<pre class="whitespace-pre-wrap text-gray-400">' + Admin.escapeHtml(JSON.stringify(obj, null, 2)) + '</pre>';
    } catch (e) {
      return '<pre class="whitespace-pre-wrap text-gray-400">' + Admin.escapeHtml(Admin.formatAdminAgentText(String(detail))) + '</pre>';
    }
  };

  // ────────────────────────────────────────────
  // formatLLMConversation  (coin-app.js version with collapsible sections)
  // ────────────────────────────────────────────
  Feed.formatLLMConversation = function (detail) {
    var obj = detail;
    try {
      if (typeof detail === 'string') obj = JSON.parse(detail);
      obj = Admin.transformAdminAgentValue(obj);
    } catch (e) {
      return '<pre class="whitespace-pre-wrap text-gray-400">' + Admin.escapeHtml(Admin.formatAdminAgentText(String(detail))) + '</pre>';
    }
    ...
    var makeCollapsible = function (role, bodyText, defaultOpen) {
      ...
      return '<div class="llm-msg ' + typeClass + '">'
        ...
        + '</div>'
        + '<div id="' + id + '" class="' + bodyClass + '">' + Admin.escapeHtml(Admin.formatAdminAgentText(bodyText)) + '</div>'
        + '</div>';
    };

    if (model) html += '<div class="llm-model-tag">' + Admin.escapeHtml(String(model)) + '</div>';
  ...
  // setLogFilter
  // ────────────────────────────────────────────
  Feed.setLogFilter = function (filter) {
  ...
  Feed.updateScrollBadge = function () {
  ...
  // ────────────────────────────────────────────
  Feed.highlightCard = function (data) {
  ...
  Feed.navigateToCard = function (data) {
    if (!data) return;
    Feed.highlightCard(data);
  };

  // ────────────────────────────────────────────
  // getActivityIdentity  (shared dedup logic)
  // ────────────────────────────────────────────
  Feed.getActivityIdentity = function (data) {
    return Admin.getActivityIdentity(data);
  };

  // ── Expose ──
  window.Admin.Feed = Feed;
})();
