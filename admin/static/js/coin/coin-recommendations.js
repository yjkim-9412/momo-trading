 ...
 *
 * Load, render, approve/reject pending recommendations,
 * and update expiry countdowns.
 *
...
(function () {
  ...
  Coin.loadRecommendations = async function () {
    try {
      var json = await Admin.fetchJSON(Admin.API + '/recommendations?status=PENDING');
      var data = json.data || [];
      Coin.renderRecommendations(data);
    } catch (err) {
  ...
  Coin.renderRecommendations = function (recs) {
    ...
    container.innerHTML = recs.map(function (r) {
      ...
      var price = Coin.formatPrice(priceValue);
      var qty = Coin.formatCoinQty(qtyValue);
      var amount = Admin.formatKRW(amountValue);
      ...
      <div class="rec-card" style="border-left:3px solid ${borderColor}; background:rgba(30,30,46,0.6); border-radius:8px; padding:10px 12px; margin-bottom:8px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
          <div>
            <span style="color:#e2e8f0; font-weight:600; font-size:14px;">${Admin.escapeHtml(coin)}</span>
  ...
  Coin.approveRecommendation = async function (id) {
    try {
      await Admin.fetchJSON(Admin.API + '/recommendations/' + id + '/approve', { method: 'POST' });
  ...
  Coin.rejectRecommendation = async function (id) {
    try {
      await Admin.fetchJSON(Admin.API + '/recommendations/' + id + '/reject', { method: 'POST' });
      Coin.loadRecommendations();
    } catch (err) {
  ...
  Coin.updateRecCountdowns = function () {
    ...
    els.forEach(function (el) {
      ...
      if (remaining <= 0) {
        ...
        el.textContent = '만료: ' + Coin.formatDuration(remaining) + ' 후';
      }
    });
  };
})();
