/**
 * app-llm.js — LLM status and usage display
 *
 * Loads and renders LLM provider status, agent configuration,
 * token usage summaries, model cards, and daily usage charts.
 */
(function () {
  'use strict';
  ...
  // ── LLM Status ──
  App.loadLLMStatus = async function () {
    try {
      var json = await Admin.fetchJSON(Admin.API + '/llm/status');
      var s = json.data;
      if (!s) return;
      var providerId = s.selected_provider || (s.tier1 && s.tier1.provider);
      var providerLabel = s.provider_name || App.formatProviderLabel(providerId);
      App.renderLLMAgentSummary(s, providerId, providerLabel);
      App.renderLLMAgentGuide(s, providerId, providerLabel);
      Admin.refreshIcons();
    } catch (err) {
  ...
  // ── LLM Usage ──
  App.loadLLMUsage = async function () {
    try {
      var json = await Admin.fetchJSON(Admin.API + '/llm/usage');
      var d = json.data;
      if (!d) {
      ...
      var providerLabel = d.provider_name || App.formatProviderLabel(d.provider || (d.app_usage && d.app_usage.provider));
      ...
      if (app && app.total_calls > 0) {
        ...
        _appendFlexRow(appEl, '\uC785\uB825 \uD1A0\uD070', App.formatTokens(totalInputTokens), 'text-gray-400', 'text-blue-400');
        _appendFlexRow(appEl, '\uCD9C\uB825 \uD1A0\uD070', App.formatTokens(totalOutputTokens), 'text-gray-400', 'text-green-400');

        if (app.session_id) {
          _appendFlexRow(appEl, '\uC138\uC158', Admin.escapeHtml(String(app.session_id).slice(0, 8)), 'text-gray-400', 'text-gray-300');
        }
        ...
        if (app.by_model && Object.keys(app.by_model).length) {
          for (var model in app.by_model) {
            ...
            modelDetail.textContent = App.formatTokens(mu.input_tokens) + ' in / ' + App.formatTokens(mu.output_tokens) + ' out';
            ...
            if (cachedTokens) {
              ...
              cachedDiv.textContent = App.formatTokens(cachedTokens) + ' cached';
      ...
      App.renderModelCards(modelData, modelsEl);
      ...
      if (dailyTokens.length) {
        ...
        totals.forEach(function (t) {
          ...
          tokSpan.textContent = App.formatTokens(t.tokens);
  ...
  App.renderModelCards = function (models, targetEl) {
    if (!targetEl) return;
    ...
    for (var model in models) {
      ...
      _addGridPair(grid, '\uC785\uB825', App.formatTokens(input), 'text-gray-400');
      _addGridPair(grid, '\uCD9C\uB825', App.formatTokens(output), 'text-green-400');
      _addGridPair(grid, '\uCE90\uC2DC\uC77D\uAE30', App.formatTokens(cached), 'text-blue-400');
      if (creation) _addGridPair(grid, '\uCE90\uC2DC\uC0DD\uC131', App.formatTokens(creation), 'text-purple-400');
  ...
  App.formatProviderLabel = function (provider) {
  ...
  // ── Agent config helper ──
  App.getLLMAgentConfig = function (status, tierKey) {
    return Object.assign({}, Admin.LLM_AGENT_DEFAULTS[tierKey], (status && status[tierKey]) || {});
  };

  App.formatReasoningEffortText = function (effort, provider) {
  ...
  function formatTier1ProfileMeta(status) {
    ...
    return orderedKeys.map(function (key) {
      ...
      return Admin.escapeHtml(label + ' ' + effort);
    }).join(' \u00b7 ');
  }

  // ── LLM Agent Summary (left sidebar settings) ──
  App.renderLLMAgentSummary = function (status, providerId, providerLabel) {
    var summaryEl = document.getElementById('llm-agent-summary');
    if (!summaryEl) return;

    summaryEl.replaceChildren();
    ['tier1', 'tier2'].forEach(function (tierKey) {
      var tierConfig = App.getLLMAgentConfig(status, tierKey);
      ...
      // formatAgentMeta returns HTML-escaped content, safe for textContent equivalent
      // but we need the middot separator so we keep it as text
      metaDiv.textContent = [providerLabel || App.formatProviderLabel(providerId), tierConfig.model || '-',
        App.formatReasoningEffortText(tierConfig.reasoning_effort, providerId)].filter(Boolean).join(' \u00b7 ');
  ...
  App.renderLLMAgentGuide = function (status, providerId, providerLabel) {
    ...
    ['tier1', 'tier2'].forEach(function (tierKey) {
      ...
      meta.textContent = [providerLabel || App.formatProviderLabel(providerId), tierConfig.model || '-',
        App.formatReasoningEffortText(tierConfig.reasoning_effort, providerId)].filter(Boolean).join(' \u00b7 ');
