/**
 * app-schedule.js — Schedule timeline and system status
 *
 * Loads and renders the adaptive rescan panel, fixed job timeline,
 * and system status display with market context bar updates.
 */
(function () {
  'use strict';
  ...
  // ── Schedule Timeline ──
  App.loadScheduleTimeline = async function () {
    try {
      var json = await Admin.fetchJSON(Admin.API + '/schedule/timeline?market=' + App.currentMarket);
      var d = json.data;
      if (!d) return;
      App.renderAdaptivePanel(d.adaptive);
      App.renderFixedTimeline(d.fixed_jobs);
    } catch (err) {
      console.error('Schedule timeline error:', err);
    }
  };

  App.renderAdaptivePanel = function (a) {
  ...
  App.renderFixedTimeline = function (jobs) {
  ...
  // ── System Status ──
  App.loadSystemStatus = async function () {
    try {
      var json = await Admin.fetchJSON(Admin.API + '/system/status?market=' + App.currentMarket);
      var s = json.data;
      if (!s) return;
      Admin.updateBadge('badge-trading', s.trading_enabled ? '\uB9E4\uB9E4:ON' : '\uB9E4\uB9E4:OFF', s.trading_enabled ? 'green' : 'red');
      Admin.updateBadge('badge-mcp', s.mcp_connected ? 'MCP:ON' : 'MCP:OFF', s.mcp_connected ? 'green' : 'red');
      ...
      if (s.last_cycle_time) {
        ...
        lastDiv.textContent = '\uB9C8\uC9C0\uB9C9: ' + Admin.formatTime(s.last_cycle_time);
      ...
      if (triggerBtn) {
        ...
        triggerBtn.appendChild(document.createTextNode(' ' + (App.enabledMarkets.length > 1 ? scopeLabel + ' ' : '') + action));
        Admin.refreshIcons();
      }
      var reportBtn = document.querySelector('[onclick="generateReport()"]');
      if (reportBtn && App.enabledMarkets.length > 1) {
  ...
  function updateMarketContextBar(statusData) {
    ...
    if (lastEl) {
      lastEl.textContent = statusData.last_cycle_time
        ? '\uB9C8\uC9C0\uB9C9 \uC0AC\uC774\uD074: ' + Admin.formatTime(statusData.last_cycle_time)
        : '';
    }
